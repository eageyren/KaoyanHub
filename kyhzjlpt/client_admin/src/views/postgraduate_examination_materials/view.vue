	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="资料名称" prop="data_name">
							  ...
							  v-if="user_group === '管理员' || (form['postgraduate_examination_materials_id'] && $check_field('set','data_name')) || (!form['postgraduate_examination_materials_id'] && $check_field('add','data_name'))" :disabled="disabledObj['data_name_isDisabled']"></el-input>
				...
				<el-form-item label="封面" prop="cover">
						...
						:show-file-list="false" v-if="user_group === '管理员' || (form['postgraduate_examination_materials_id'] && $check_field('set','cover')) || (!form['postgraduate_examination_materials_id'] && $check_field('add','cover'))">
				...
				<el-form-item label="资料类型" prop="data_type">
						...
						v-if="user_group === '管理员' || (form['postgraduate_examination_materials_id'] && $check_field('set','data_type')) || (!form['postgraduate_examination_materials_id'] && $check_field('add','data_type'))">
				...
				<el-form-item label="知识点" prop="knowledge_points">
							  ...
							  v-if="user_group === '管理员' || (form['postgraduate_examination_materials_id'] && $check_field('set','knowledge_points')) || (!form['postgraduate_examination_materials_id'] && $check_field('add','knowledge_points'))" :disabled="disabledObj['knowledge_points_isDisabled']"></el-input>
				...
				<el-form-item label="资料文件" prop="information_documents">
					...
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['postgraduate_examination_materials_id'] && $check_field('set','information_documents')) || (!form['postgraduate_examination_materials_id'] && $check_field('add','information_documents'))" class="upload-demo" drag
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/postgraduate_examination_materials/add?",
				url_set: "~/api/postgraduate_examination_materials/set?",
				url_get_obj: "~/api/postgraduate_examination_materials/get_obj?",
				url_upload: "~/api/postgraduate_examination_materials/upload?",

				query: {
					"postgraduate_examination_materials_id": 0,
				},
				...
				form: {
								"data_name":  '', // 资料名称
										...
										"information_documents":  '', // 资料文件
											"postgraduate_examination_materials_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/postgraduate_examination_materials/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/postgraduate_examination_materials/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/postgraduate_examination_materials/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/postgraduate_examination_materials/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/postgraduate_examination_materials/view','get');
