        ...
        <div class="warp" v-if="$check_exam('/exam/list', 'answers')">
            <div class="container exam-detail">
                <div class="row">
                    ...
                    <div v-for="(o,i) in question_list" :key="o.exam_question_id || i">
        ...
        methods: {
            async get_obj_after(){
                ...
                if(answerJson.result && answerJson.result.obj != null){
                    ...
                    // 使用安全的JSON解析方法
                    this.answer_form.answers = this.safeJSONParse(this.answer_form.answers);
                    this.answer_form.score_detail = this.safeJSONParse(this.answer_form.score_detail);
            ...
            submit_main(){
                ...
                for (let i=0;i<_this.answers.length;i++){
                    // 清理答案中可能存在的控制字符
                    if (typeof _this.answers[i] === 'string') {
                        _this.answers[i] = _this.answers[i].replace(/[\x00-\x1F\x7F-\x9F]/g, '');
                    } else if (Array.isArray(_this.answers[i])) {
                        _this.answers[i] = _this.answers[i].map(item => {
                ...
                // 使用安全的JSON字符串转换
                _this.answer_form.answers = this.safeJSONStringify(_this.answer_form.answers);
                _this.answer_form.score_detail = this.safeJSONStringify(_this.answer_form.score_detail);
