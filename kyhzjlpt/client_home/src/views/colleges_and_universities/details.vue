<template>
	<div class="diy_details page_colleges_and_universities" id="colleges_and_universities_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_colleges_and_universities'>
							<div_colleges_and_universities :obj="obj"></div_colleges_and_universities>
...
<script>
	import div_colleges_and_universities from "@/components/diy/div_colleges_and_universities.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "colleges_and_universities_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/colleges_and_universities/set?colleges_and_universities_id=' + obj["colleges_and_universities_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "colleges_and_universities",
							source_field: "colleges_and_universities_id",
							source_id: this.obj.colleges_and_universities_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.colleges_and_universities_id;
				}
			},
			/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "colleges_and_universities",
					source_field: "colleges_and_universities_id",
					source_id: obj["colleges_and_universities_id"],
					user_id
				};
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.name_of_institution
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/colleges_and_universities/set?colleges_and_universities_id=' + _this.obj["colleges_and_universities_id"], {
								praise_len
							}, (res) => {
								if(res.result){
